  ...
  const gaugeData = [
    {
      ...
      itemStyle:{
        color: {
          ...
          colorStops: [
            {
              offset: 0, color: 'rgb(147,114,241)'
            },
            {
              offset: 1, color: 'rgba(167,142,242,.6)' // 100% 处的颜色
            }
          ]
        },
        global: false
  ...
  const gaugeData = [
    {
      ...
      itemStyle:{
        color: {
          ...
          colorStops: [
            {
              offset: 0, color: 'rgba(147,114,241,0)'
            },
            {
              offset: 1, color: 'rgba(167,142,242,0)' // 100% 处的颜色
            }
          ]
        },
        global: false
...
<template>
 <div class="content">
    ...
    <div class="warningInfo">
      ...
      <img v-show="warningStatus==1" src="../assets/img/zhengchang.png" alt="">
      <div class="warningText">
        <div>检测状态</div>
        <div :class="warningStatus==0?'weizhi':(warningStatus==1?'zhengchang':'baojing')">{{warningStatus==0?'未知':warningStatus==1?'正常':'报警'}}</div>
      </div>
    </div>
    <slider :min=0 :max=100 :pieData="pieData" class="scrollLine" @getWarningNum="getWarningNum"></slider>
 </div>
</template>
